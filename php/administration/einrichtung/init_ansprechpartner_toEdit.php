...
if($session->checkSessionVariables('username', 'usergroup')){
    ...
    if (!$benutzer->isAdmin($session->usergroup)) {
        ...
        $output ='';
        if(isset($_POST['einrichtung'])){
            require_once('../../utilities/db_connection.php');
            $database = new DatabaseConnection();
            $conn = $database->getConn();
            $sql = "SELECT * FROM keis2_ansprechpartner WHERE einrichtung_id =
            '".$conn->real_escape_string($_POST['einrichtung'])."'";
                    ...
                    $output .= '
                        <tr id="'.$row['id'].'">
    				        ...
    				        <td style="text-align:center">
					        	<a class="btn btn-danger btn-circle custom3" href="#" id="btn_remove_ansprechpartner"><span class="fa fa-trash-alt" title="löschen" aria-hidden="true"></span></a>
    				        </td>
                            <td>'.$row['vorname'].'</td>
                            <td>'.$row['name'].'</td>
                            <td>'.$rechnung.'</td>
