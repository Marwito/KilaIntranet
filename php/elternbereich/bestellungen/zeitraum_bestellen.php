...
if(isset($_POST['datetimepicker1'], $_POST['datetimepicker2'], $_POST['kind'], $_POST['essenkategorie'])){
        ...
        if($day != 'Sat' && $day != 'Sun' && !$connector->isFeiertagInLand(date('Y-m-d',$date), LPLib_Feiertage_Connector::LAND_BAYERN)) {
            ...
            if($dauerbesteller && $abbestellung[$weekday] && $date >= $essenstart && $date <= $essenende+1) {
                ...
                if($resultCheck->num_rows > 0) {
                    $sqlUpdate = "UPDATE keis2_abbestellung SET update_date=
                                  NOW()
                    ...
                    if (!$conn->query($sqlUpdate)=== TRUE) {
                        throw new Exception("Beim Bestellen der Essen ist ein Datenbankfehler aufgetreten!");
                    }
                }
            } else if(!$dauerbesteller){
                $resultCheck = $conn->query($sqlCheckBestellung.date("Y-m-d", $date)."';");
                if($resultCheck->num_rows == 0) {
                    $sqlInsert = "INSERT INTO keis2_bestellung (kind, datum, essenkategorie, insert_date)
    ...
    echo "Die Essen wurden erfolgreich bestellt.";
} else {
