 ...
 *	$is_feiertag = $connector->isFeiertagInLand('2014-01-01', LPLib_Feiertage_Connector::LAND_BAYERN);
...
{
    const BASE_URL_FEIERTAG_API = 'https://feiertage-api.de/api/';
    ...
    public function getFeiertageVonLand($jahr,$land)
    {
        if(isset(self::$cache['c0-'.$jahr.$land]))
            ...
            $result = $this->_file_get_contents_t_curl(LPLib_Feiertage_Connector::BASE_URL_FEIERTAG_API.'?jahr='.$jahr.'&nur_land='.$land);
    ...
    public function getAlleFeiertage($jahr)
    {
        if(isset(self::$cache['c1-'.$jahr]))
            ...
            $result = $this->_file_get_contents_t_curl(LPLib_Feiertage_Connector::BASE_URL_FEIERTAG_API.'?jahr='.$jahr);
    ...
    public function getAlleFeiertageOhneLaender($jahr)
    {
        if(isset(self::$cache['c2-'.$jahr]))
            ...
            $result = $this->_file_get_contents_t_curl(LPLib_Feiertage_Connector::BASE_URL_FEIERTAG_API.'?jahr='.$jahr.'&nur_daten=1');
